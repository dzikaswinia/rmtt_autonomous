...
class State:
    ...
    # TODO check that degrees never get under 0 and over 360

    def update_coordinate(self,coord, val):
        if (coord == 3):
            print(f'checking degress')
    ...
    def update(self, cmd):
        print(f'[state_tracker | update] Updating the position with command: {cmd.name}'
        ...
        if (cmd.name == "forward"):
            val = [x for x in CMDS if x[0] == "forward"]
       # print(f'Cmd {cmd.name} with val:{val}')
...
#cmd_f = command.Command("forward", "22")
